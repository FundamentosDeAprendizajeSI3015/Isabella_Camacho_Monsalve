...
from src.kernel_metrics import FSM, complex_ratio, kernel_aligment, kernel_polarization
...
def run_simulation(iteraciones, numero_k, t, x, y):
    """
    ...
    for k in range(iteraciones):
        ...
        KL_train, _ = create_weak_kernels(
            ...
            X_test=X_test,
            max_features=t,
            num_kernels=numero_k,
        )
        
        # Train models and compute kernel metrics
        natural_metrics, anti_natural_metrics, rbf_metrics, poly_metrics = train_and_evaluate(
            KL_train, X_train, y_train
        )
...
def iteractions_simulation(iteraciones, numero_k, t, x, y):
    """Backward-compatible wrapper for previous misspelled API."""
    return run_simulation(iteraciones, numero_k, t, x, y)