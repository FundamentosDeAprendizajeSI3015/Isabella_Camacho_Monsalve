...
# Temporal split: Train on early years, Validate on middle years, Test on recent years
print("\n===== TEMPORAL DATA SPLIT (Time-Based) =====")
years = sorted(df['anio'].unique())
n_years = len(years)

if n_years < 3:
    print("Warning: Small number of years. Using 60% train, 20% val, 20% test temporal split")
    train_years = years[:max(1, n_years//3)]
    val_years = years[max(1, n_years//3):max(2, 2*n_years//3)]
    test_years = years[max(2, 2*n_years//3):]
else:
    train_years = years[:n_years//3]
    val_years = years[n_years//3:2*n_years//3]
    test_years = years[2*n_years//3:]
...
df_with_indices = df_reduced.copy()
df_with_indices['original_index'] = range(len(df_with_indices))
df_with_indices['anio_temp'] = df['anio'].values

train_idx = df_with_indices[df_with_indices['anio_temp'].isin(train_years)]['original_index'].values
val_idx = df_with_indices[df_with_indices['anio_temp'].isin(val_years)]['original_index'].values
test_idx = df_with_indices[df_with_indices['anio_temp'].isin(test_years)]['original_index'].values
...
# Additional imputation to handle any remaining NaN values before scaling
...
X_val_imputed[NUM_COLS] = imputer.transform(X_val[NUM_COLS])
X_test_imputed[NUM_COLS] = imputer.transform(X_test[NUM_COLS])

print(f"Imputed missing values using median strategy")
print(f"Remaining NaN in train: {X_train_imputed.isnull().sum().sum()}")
print(f"Remaining NaN in val: {X_val_imputed.isnull().sum().sum()}")
print(f"Remaining NaN in test: {X_test_imputed.isnull().sum().sum()}")
...
X_val_scaled[NUM_COLS] = scaler.transform(X_val_imputed[NUM_COLS])
...
print(f"  - Temporal split: Train earliest years → Validation middle years → Test recent years")
print(f"  - Class balancing: Applied if class ratio > 1.5")
